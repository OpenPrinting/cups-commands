  ...
  for (i = 1; i < argc; i ++)
  {
        ...
        switch (*opt)
	{
	  ...
	  case 'd' : // Destination printer or class
		...
		if (i >= argc)
		{
		  cupsLangPrintf(stderr, _("%s: Error - expected destination after \"-d\" option."), argv[0]);
		  usage();
		}
	  ...
	  case 'f' : // Form
		...
		if (i >= argc)
		{
		  cupsLangPrintf(stderr, _("%s: Error - expected form after \"-f\" option."), argv[0]);
		  usage();
		}
	      }

	      cupsLangPrintf(stderr, _("%s: Warning - form option ignored."), argv[0]);
	      break;
	  ...
	  case 'i' : // Change job
		...
		if (i >= argc)
		{
		  cupsLangPrintf(stderr, _("%s: Expected job ID after \"-i\" option."), argv[0]);
		  usage();
		}
		...
		val = argv[i];
	      ...
	      if (num_files > 0)
	      {
		cupsLangPrintf(stderr, _("%s: Error - cannot print files and alter jobs simultaneously."), argv[0]);
		return (1);
	      }
		...
		job_id = atoi(val);
	      ...
	      if (job_id < 0)
	      {
		cupsLangPrintf(stderr, _("%s: Error - bad job ID."), argv[0]);
		break;
	      }
	      break;
	  ...
	  case 'n' : // Number of copies
		...
		if (i >= argc)
		{
		  cupsLangPrintf(stderr, _("%s: Error - expected copies after \"-n\" option."), argv[0]);
		  usage();
		}
	  ...
	  case 'q' : // Queue priority
		...
		if ((i + 1) >= argc)
		{
		  cupsLangPrintf(stderr, _("%s: Error - expected priority after \"-%c\" option."), argv[0], *opt);
		  usage();
		}
	  ...
	  case 't' : // Title
		...
		if (i >= argc)
		{
		  cupsLangPrintf(stderr, _("%s: Error - expected title after \"-t\" option."), argv[0]);
		  usage();
		}
	  ...
	  case 'y' : // mode-list
		...
		if (i >= argc)
		{
		  cupsLangPrintf(stderr, _("%s: Error - expected mode list after \"-y\" option."), argv[0]);
		  usage();
		}
	      }

	      cupsLangPrintf(stderr, _("%s: Warning - mode option ignored."), argv[0]);
	      break;
	  ...
	  case 'H' : // Hold job
		...
		if (i >= argc)
		{
		  cupsLangPrintf(stderr, _("%s: Error - expected hold name after \"-H\" option."), argv[0]);
		  usage();
		}
		...
		if (job_id < 1)
		{
		  cupsLangPrintf(stderr, _("%s: Need job ID (\"-i jobid\") before \"-H restart\"."), argv[0]);
		  return (1);
		}
	  ...
	  case 'P' : // Page list
		...
		if (i >= argc)
		{
		  cupsLangPrintf(stderr, _("%s: Error - expected page list after \"-P\" option."), argv[0]);
		  usage();
		}
	  ...
	  case 'S' : // character set
		...
		if (i >= argc)
		{
		  cupsLangPrintf(stderr, _("%s: Error - expected character set after \"-S\" option."), argv[0]);
		  usage();
		}
	      }

	      cupsLangPrintf(stderr, _("%s: Warning - character set option ignored."), argv[0]);
	      break;
	  ...
	  case 'T' : // Content-Type
		...
		if (i >= argc)
		{
		  cupsLangPrintf(stderr, _("%s: Error - expected content type after \"-T\" option."), argv[0]);
		  usage();
		}
	      }

	      cupsLangPrintf(stderr, _("%s: Warning - content type option ignored."), argv[0]);
	      break;
...
usage(void)
{
  ...
  cupsLangPuts(stdout, _("-H HH:MM                Hold the job until the specified UTC time"));
  cupsLangPuts(stdout, _("-H hold                 Hold the job until released/resumed"));
  cupsLangPuts(stdout, _("-H immediate            Print the job as soon as possible"));
  cupsLangPuts(stdout, _("-H restart              Reprint the job"));
  cupsLangPuts(stdout, _("-H resume               Resume a held job"));
  cupsLangPuts(stdout, _("-i id                   Specify an existing job ID to modify"));
  cupsLangPuts(stdout, _("-m                      Send an email notification when the job completes"));
  cupsLangPuts(stdout, _("-n num-copies           Specify the number of copies to print"));
  ...
  cupsLangPuts(stdout, _("-P page-list            Specify a list of pages to print"));
  cupsLangPuts(stdout, _("-q priority             Specify the priority from low (1) to high (100)"));
  cupsLangPuts(stdout, _("-s                      Be silent"));
